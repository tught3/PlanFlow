...
class _ConfirmScreenState extends State<ConfirmScreen>
  ...
  bool _showInitialHydrationLoader = false;
  ...
  void initState() {
    ...
    // parse_pending 진입은 GPT hydrate가 끝날 때까지 편집기를 숨긴다.
    // 첫 프레임부터 로더를 표시해 로컬 초안이 잠깐 노출되지 않게 한다.
    _showInitialHydrationLoader =
        widget.parsedSchedule['parse_pending'] == true &&
            rawTextForLocalParse != null &&
            rawTextForLocalParse.isNotEmpty;
    ...
    _strongAlarm = widget.parsedSchedule['use_strong_alarm'] == true;
    if (_strongAlarm) _isCritical = true;
  ...
  Future<void> _hydrateParsedSchedule(String rawText) async {
    ...
    try {
      ...
      setState(() {
        ...
        if (!_endEditedByUser) {
          _endAt = _safeEndAt(parsed['end_at'], _startAt, rawText: rawText);
        }
        if (parsed['is_critical'] == true) {
          _isCritical = true;
        }
        if (parsed['use_strong_alarm'] == true) {
          _strongAlarm = true;
          _isCritical = true;
        }
      });
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (needsBackgroundLocationResolve) {
        unawaited(
          _resolveLocationCoordinatesAfterSave(
  ...
  int? _sinoKoreanTensFallback(String text) {
    ...
    final tenMatch = RegExp(
      r'^(?:(일|이|삼|사|오|육|륙|칠|팔|구)?십)?(일|이|삼|사|오|육|륙|칠|팔|구)?$',
    ).firstMatch(text);
