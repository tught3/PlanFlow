...
class VoiceCommandPipeline {
  ...
  List<String> extractRequestedChanges(String text) {
    ...
    if (RegExp(
      r'(시간|시각|언제|몇\s*시|오전|오후|아침|점심|저녁|밤|오늘|내일|모레|글피|이번\s*주|다음\s*주|이번주|다음주|[월화수목금토일]요일|(?:그\s*)?다음\s*날|'
      r'(?:하루|이틀|사흘|나흘|닷새|엿새|이레|여드레|아흐레|열흘|삼일|\d+\s*일)\s*(?:뒤|후|전|앞)|'
      r'일주일\s*(?:뒤|후|전|앞)|한\s*주(?:일)?\s*(?:뒤|후|전|앞)|\d{1,2}\s*주(?:일)?\s*(?:뒤|후|전|앞)|'
      r'한\s*달\s*(?:뒤|후|전|앞)|\d{1,2}\s*(?:개월|달|월)\s*(?:뒤|후|전|앞)|'
      r'연기|미뤄|밀어|옮겨|이동|앞당겨|늦춰|당겨|바꿔|변경|수정)',
    ).hasMatch(normalized)) {
  ...
  RegExpMatch? _lastDateTimeValueMatch(String text) {
    final patterns = <RegExp>[
      RegExp(
        r'((?:그\s*)?다음\s*날(?:로|에|으로)?|'
        r'(?:하루|이틀|사흘|나흘|닷새|엿새|이레|여드레|아흐레|열흘|삼일|\d+\s*일)\s*(?:뒤|후)(?:로|에|으로)?|'
        r'(?:하루|이틀|사흘|나흘|닷새|엿새|이레|여드레|아흐레|열흘|삼일|\d+\s*일)\s*(?:전|앞)(?:으로|로|에)?|'
        r'일주일\s*(?:뒤|후)(?:로|에|으로)?|한\s*주(?:일)?\s*(?:뒤|후)(?:로|에|으로)?|\d{1,2}\s*주(?:일)?\s*(?:뒤|후)(?:로|에|으로)?|'
        r'한\s*달\s*(?:뒤|후)(?:로|에|으로)?|\d{1,2}\s*(?:개월|달|월)\s*(?:뒤|후)(?:로|에|으로)?)',
      ),
      RegExp(
