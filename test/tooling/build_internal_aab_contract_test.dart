...
void main() {
  ...
  test('raw release Gradle path rejects missing or placeholder map defines',
      () {
    expect(gradleScript.existsSync(), isTrue);

    final source = gradleScript.readAsStringSync().replaceAll('\r\n', '\n');
    expect(source, contains('releaseBuildRequested'));
    expect(source, contains('isPlaceholderMapDefine'));
    expect(source, contains('your-google-maps-api-key'));
    expect(source, contains('GOOGLE_MAPS_API_KEY'));
    expect(source, contains('TMAP_API_KEY'));
    expect(source, contains('NAVER_MAP_CLIENT_ID'));
    expect(
      source,
      contains('Release map dart-defines are missing or placeholders'),
    );
  });
}
