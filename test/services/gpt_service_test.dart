...
void main() {
  ...
  group('GptService', () {
    test('defaults an AI-supplied time to 09:00 when the request has no time',
        () async {
      ...
      final result = await service.parseSchedule('프로젝트 회의 일정으로 저장');

      expect(
        result['start_at'],
        DateTime(reference.year, reference.month, reference.day, 9)
            .toIso8601String(),
      );
    });
    ...
    test('locally infers all-day, multi-day, category, and recurrence hints',
      ...
      final monthlyAction = await service.parseSchedule('매월 1일 톨비 작성 반복');
