...
void main() {
  ...
  test('resolveHomeWidgetRoute maps voice launcher and auto-start routes', () {
    ...
    expect(
      resolveHomeWidgetRoute(Uri.parse('planflow-v2://voice-launcher?source=mic')),
      '${AppRoutes.voice}?autoStart=1',
    );
    expect(
  ...
  test('resolveHomeWidgetRoute maps calendar and event deep links', () {
    expect(
      resolveHomeWidgetRoute(Uri.parse('planflow-v2://calendar?date=2026-05-21')),
    ...
    expect(
      resolveHomeWidgetRoute(Uri.parse('planflow-v2://event?eventId=event-456')),
