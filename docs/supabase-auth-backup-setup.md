...
> **주의**: `http://127.0.0.1:3000/**` 같은 와일드카드 로컬호스트 항목은 개발 프로젝트 한정으로만 사용하세요. 가능하면 실제 사용하는 정확한 포트/경로로 한정하고, 운영(프로덕션) Supabase 프로젝트의 Redirect URLs 허용목록에는 이 와일드카드 항목을 프로덕션 배포 전 제거하는 것을 검토해야 합니다.
