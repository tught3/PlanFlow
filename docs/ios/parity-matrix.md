# Android → iOS 기능 동등성 매트릭스 (Phase 2)
...
| 개인 위젯 | `widget_schedule_contract.dart`, `ios/PlanFlowWidget` | CLOUD_MACOS_REQUIRED |
...
제공하고, Android 기존 위젯은 이를 계속 소비한다는 수준이다. WidgetKit은
`NEAR_PARITY` 후보이지만 Xcode target 연결 전까지는 실제 렌더러로 판정하지 않는다.
...
공휴일 순서를 포함한다. 앱과 Android 위젯의 기존 동작을 대체하지 않고 iOS 소비자를
추가할 때 사용할 정규 JSON이다. `NotificationRouteContract`는 기존 `planflow://` 스킴을
