...
---
<!-- archived 2026-09-06 08:27 by generate-agents-md.ps1 (line-budget cleanup) -->
...
-
  ```perl
  $text=(& powershell.exe @args 2>&1 | ForEach-Object {[string]## FluxOS 파이프라인 연결
이 세션의 작업 지시는 FluxOS 파이프라인을 통해 처리된다.
...
- **비단순 작업의 모든 단계 경계(조사→구현, 구현→리뷰, 리뷰→재작업, 재작업→완료)에서 **반드시 멈추고 CEO에게 다음 단계용 모델 추천(모델명 + thinking 레벨)을 보고**한다. CEO가 직접 모델을 바꾼 뒤 다음 단계를 진행한다. 추천 모델 풀은 **glm5.3 / glm5.3플래시 / m3 세 가지로 한정**하고, 추천 기준은 **현재 작업을 1회에 완벽히 수행 가능한 모델+thinking 조합 중 가장 저렴한 것**이다.**
  - Why(2026-09-04 CEO 결정): "지시를 주면 조사 끝나고 계획 세워지면 멈추고 어떤 모델이 좋을지 추천해주면 내가 그 모델로 바꿔서 구현을 시키고 구현이 다 끝나면 다시 멈추고 검토모델로 바꿔서 리뷰시켜서 ... 기준은 1회에 완벽하게 구현할 수 있는 모델+추론정도 중 가장 저렴한 모델로 추천."

같은 날 확정된 audit_then_stop_handoff_for_cheaper_implementation이 조사→구현 경계의 1단계를 잡았고, 이 preference는 그 뒤 단계(구현→리뷰, 리뷰→재작업)도 동일 패턴으로 일관 적용해 비용 최적화와 CEO 통제권을 양립시키는 완결판이다. 풀을 3개로 좁힌 이유: (a) 풀 확장은 추천 비교 비용을 키우고 (b) CEO가 매번 모델을 직접 전환해야 하므로 의사결정 부담이 가중된다. 같은 3개 모델 안에서 thinking 레벨로 성능/비용을 미세조정하는 게 실용적이다.

## How to apply

**단계별 동작 (멈춤 시점 + 추천 보고 형식):**

1. **조사 + 계획 단계** — 현재 세션 모델(기본 MiniMax-M3)로 진행. 인계 문서(감사 확정 사실 + 설계 + REQUIRED_CHECKS + 완료 조건) 작성 후 멈춤. 추천 보고:
   ```
   [구현 단계 진입]
   - 추천 모델: <glm5.3플래시 | m3 | glm5.3>
   - 추천 thinking: <none | low | medium | high>
   - 추천 근거: <왜 그 모델+레벨이 1회 구현 가능한가>
   - 작업 요약: <범위 1줄>
   ```

2. **구현 단계 완료 → 리뷰 진입 멈춤** — 구현 모델이 결과 보고하면 멈춤. 추천 보고:
   ```
   [리뷰 단계 진입]
   - 추천 모델: <...>
   - 추천 thinking: <...>
   - 추천 근거: <리뷰는 구현과 다른 사고 깊이가 필요해서 ...>
   - 리뷰 초점: <계약 정합 / 회귀 / 규약 위반 / 성능 등>
   ```

3. **리뷰 후 재작업 필요 시 멈춤** — 재작업 항목이 있으면 멈춤. 추천 보고:
   ```
   [재작업 필요]
   - 재작업 항목: <리스트>
   - 추천 모델: <...>
...
4. **완료 보고** — 모든 단계 통과 + 커밋(pathspec) + push + REQUIRED_CHECKS 전부 exit 0 + git status clean 확인 후 "완료" 보고.

**추천 풀 매핑:**
- glm5.3 = zai-coding-plan/GLM-5.2
- glm5.3플래시 = deepseek/deepseek-v4-flash
- m3 = minimax-coding-plan/MiniMax-M3

**추천 기준 (cheapest 1-shot perfect):**
1. 현재 작업을 1회에 완벽 수행 가능한 후보 모델 집합을 식별
2. 후보 안에서 (모델 단가 × 추론 토큰 예상 비용)이 가장 싼 조합 선택
3. thinking 레벨은 그 모델이 1회 완성에 필요한 **최소 레벨** (none < low < medium < high)
4. 작업 복잡도 추정 가이드:
   - 단순 1-2파일 문자열/문서 변경 → glm5.3플래시 / thinking none
   - 단일 파일 로직 변경 → glm5.3플래시 / thinking low 또는 m3 / thinking low
   - 다중 파일 로직 변경 → m3 / thinking medium
   - 아키텍처/스키마/리팩토링 → glm5.3 / thinking medium 또는 high
   - 리뷰(독립 검토) → m3 / thinking high 또는 glm5.3 / thinking high (구현과 다른 깊이)
   - 재작업(범위 한정) → glm5.3플래시 / thinking low (작은 범위) 또는 m3 / thinking medium (큰 범위)

**이 preference 활성 시 비활성화되는 글로벌 규칙:**
- pipeline_workflow_glm_plan_flash_m3_impl_m3_review (자동 4단계 파이프라인)
- glm-worker-pipeline-auto (GLM 자동 worker dispatch)
- worker-watchdog-policy의 자동 dispatch 부분
- 자동 마무리 시퀀스 (auto-finish-after-each-task) — 작업 단위
- 1N/2N 알람 자동 체크 — 단계 사이 멈춤이 명시적 체크포인트 역할

**그대로 유지되는 규칙 (안전/감사):**
- plan_gate / safety_gate (안전 게이트)
- pathspec 커밋 (git add -A 금지)
- REQUIRED_CHECKS exit 0 검증
- prevent capture (코드 수정 후 재발 방지)
- CEO 액션 아이템 시각 분리

**예외:**
- CEO가 명시적으로 "원래대로 자동" / "끝까지 자동으로" / "파이프라인 그대로" 등으로 opt-out 하면 그 작업 한정 비활성
- 단순 Q&A·읽기 전용 조사는 단계 멈춤 적용 안 함
- 풀에 없는 모델(DeepSeek Pro, GPT-5.4 Mini 등)이 정말 필요한 작업은 CEO에게 별도 확인
...
## Path
E:\FluxStudio\PlanFlow-AppsInToss

## Current State
- Initial registration needed

## AGENTS
- Write the project rules here.
