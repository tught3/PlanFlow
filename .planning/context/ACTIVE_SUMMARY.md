...
- 검증: `flutter analyze --no-pub`, focused service tests, `git diff --check`, guarded release APK, guarded split-per-ABI release APK 통과. `scripts/flutter-local.ps1 analyze --no-pub`는 worktree 상위 `.fluxos` bootstrap 부재로 Flutter 실행 전 실패했다.
