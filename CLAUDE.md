...
> 2. **계획 = `Claude Opus`(상위 모델).** 범위·영향파일·리스크·검증기준 먼저 제시.
> 3. **구현 = 난이도별 병렬 서브에이전트 위임.** 단순/보일러플레이트 = `Claude Haiku`(경량), 난도 높음 = `Claude Sonnet`(중간). 파일 비중첩이면 동시 실행.
> 4. **별도 리뷰어(`Claude Sonnet`)가 전체 diff를 리뷰** — 계약 정합·회귀·규약 위반 점검. (구현 워커와 다른 별도 세션)
