...
create table if not exists public.group_members (
  ...
  left_at timestamptz generated always as (removed_at) stored,
...
create table if not exists public.group_role_delegations (
  id uuid primary key default gen_random_uuid(),
  group_id uuid not null references public.groups (id) on delete cascade,
  delegator_user_id uuid references public.users (id) on delete set null,
...
create table if not exists public.user_settings (
  ...
  voice_conversation_daily_free_date date,
  voice_conversation_daily_free_used integer not null default 0,
  voice_conversation_last_session_id text,
  created_at timestamptz not null default now()
);
...
-- 정책: 최초 누적 3회 무료 -> 소진 후 매일 1회 무료(KST 기준 매일 리셋) ->
-- 그마저 소진되면 리워드광고 1편 = 1세션.
-- ============================================================================
...
-- 13. group_event_reports (공유 그룹 일정 신고)
